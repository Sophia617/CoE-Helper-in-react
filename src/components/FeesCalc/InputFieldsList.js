...
const InputFieldsList = ({sumChanged}) => {
    ...
    const [inputGroupDisplays, setInputGroupDisplays] = useState([{id:Math.random()*1000}]);
    ...
    const inputTextHandler = (e, id)=> {
        ...
        let index = copiedUserInputValues.map(userInputs => userInputs.id).indexOf(id);
    ...
    const calculateTotalFees = (id, userInputValuesPerRow) => {
        let totalSum = {
            id: id,
            subjectFees: (userInputValuesPerRow.subjectFees * userInputValuesPerRow.subjectNumbers).toFixed(2)
        };
        let copiedTotalSumArray = [...totalFeesArray];
        let index = copiedTotalSumArray.map(totalSumItem => totalSumItem.id).indexOf(id);
        if (index === -1) {
            copiedTotalSumArray = [
                ...totalFeesArray,
                totalSum
    ...
    const calculateTotalSum = () => {
        const copiedTotalSumArray = [...totalFeesArray].map(item=> parseFloat(item.subjectFees));
    ...
    const deleteInputGroup =(id) => {
        ...
        let inputDisplayIndex =  copiedInputGroupDisplayList.map(inputGroup => inputGroup.id).indexOf(id);
        let userInputValuesIndex =  copiedUserInputValues.map(userInputValues => userInputValues.id).indexOf(id);
        let totalSumIndex =  copiedTotalSumArray.map(totalSumItem => totalSumItem.id).indexOf(id);
    ...
    return (
        <Form>
            { inputGroupDisplays.map ((inputGroup)=> {
                return <InputGroups id={inputGroup.id}  key={inputGroup.id}
